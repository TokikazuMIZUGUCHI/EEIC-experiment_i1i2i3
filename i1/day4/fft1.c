...
void pitch(complex double *Y, long n, float pitch){
  complex double * Ydash = calloc(sizeof(complex double), n);
  for(int i = 0; i < n; i++){
    if(((int)(float)i*pitch) < n){
      Ydash[(int)((float)i*pitch)] = Y[i];
    }
  }
